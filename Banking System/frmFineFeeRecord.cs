...
{
    public partial class frmGrantRecord : DevComponents.DotNetBar.Office2007Form
    {
        ...
        private void DGV_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                frm.txtPaymentModeDetails.Text = dr.Cells[7].Value.ToString();
                frm.txtTotalPaid.Value = Convert.ToInt32(dr.Cells[6].Value);
        ...
        private void buttonX3_Click(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(MemberID)[Member ID], RTRIM(Year)[Year], RTRIM(Months)[Months], RTRIM(Date)[Payment Date],RTRIM(CashierName)[Recieved By], RTRIM(FineFee)[Ammount Paid], RTRIM(Reason)[Reason]from Fines where  Date between @date1 and @date2 order by Date", con);
