...
{
    public partial class frmClientDetails : DevComponents.DotNetBar.Office2007RibbonForm
    {
        ...
        public void loadpay()
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select RTRIM(AccountNumber)[Account Number],RTRIM(AccountNames)[Account Names] from Account where Cleared='Yes' order by ID DESC", con);
        ...
        private void textBoxX1_TextChanged(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("select RTRIM(AccountNumber)[Account Number],RTRIM(AccountNames)[Account Names] from Account where AccountNumber like '" + textBoxX1.Text + "%' OR AccountNames Like '" + textBoxX1.Text + "%'  order by ID DESC", con);
                SqlDataAdapter myDA = new SqlDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "Account");
                dataGridView1.DataSource = myDataSet.Tables["Account"].DefaultView;
