...
{
    public partial class frmAnnualSubscriptionFeePaymentRecord : DevComponents.DotNetBar.Office2007Form
    {
        ...
        public frmAnnualSubscriptionFeePaymentRecord()
        {
            InitializeComponent();
        }
        ...
        private void cmbStaffName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(MemberID)[Member ID], RTRIM(Year)[Year], RTRIM(Months)[Months], RTRIM(Date)[Payment Date],RTRIM(CashierName)[Recieved By], RTRIM(AnnualAmmount)[Ammount Paid], RTRIM(Duepayment)[Due Payment] from AnnualFeesPayment where MemberID= '" + cmbStaffName.Text + "' order by ID Desc", con);
                SqlDataAdapter myDA = new SqlDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "AnnualFeesPayment");
                DGV.DataSource = myDataSet.Tables["AnnualFeesPayment"].DefaultView;
                con.Close();
            }
            catch (Exception ex)
            {
        ...
        private void AutocompleteStaffName()
        {
            try
            {
                ...
                adp.SelectCommand = new SqlCommand("SELECT distinct RTRIM(MemberID) FROM AnnualFeesPayment", CN);
                ds = new DataSet("ds");
                adp.Fill(ds);
                dtable = ds.Tables[0];
        ...
        private void buttonX3_Click(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(MemberID)[Member ID], RTRIM(Year)[Year], RTRIM(Months)[Months], RTRIM(Date)[Payment Date],RTRIM(CashierName)[Recieved By], RTRIM(AnnualAmmount)[Ammount Paid], RTRIM(Duepayment)[Due Payment] from AnnualFeesPayment where  Date between @date1 and @date2 order by Date", con);
                ...
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "AnnualFeesPayment");
                dataGridView1.DataSource = myDataSet.Tables["AnnualFeesPayment"].DefaultView;
                con.Close();
            }
            catch (Exception ex)
            {
