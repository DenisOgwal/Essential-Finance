...
{
    public partial class frmMaximumShares : DevComponents.DotNetBar.Office2007Form
    {
        ...
        private void buttonX2_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please enter Maximum Shares", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                interestrate.Focus();
                return;
            }
            try
            {
                ...
                con.Open();
                string cb = "insert into MaximumShares(Date,MaxShares,RegisteredBy) VALUES (@d1,@d2,@d4)";
                cmd = new SqlCommand(cb);
                cmd.Connection = con;
                cmd.Parameters.Add(new SqlParameter("@d1", System.Data.SqlDbType.NChar, 20, "Date"));
                cmd.Parameters.Add(new SqlParameter("@d2", System.Data.SqlDbType.Int, 10, "MaxShares"));
                cmd.Parameters.Add(new SqlParameter("@d4", System.Data.SqlDbType.NChar, 50, "RegisteredBy"));
                cmd.Parameters["@d1"].Value = date1.Text;
                cmd.Parameters["@d2"].Value = interestrate.Value;
                cmd.Parameters["@d4"].Value = chairperson.Text;
                cmd.ExecuteNonQuery();
                con.Close();
        ...
        private void buttonX3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con = new SqlConnection(cs.DBConn);
                con.Open();
                string cq = "delete  from  MonthlyFee where Months=@DELETE1 and Year=@delete2 and MultipleDate=@date3;";
        ...
        private void buttonX4_Click(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                string cb = "update MonthlyFee set MonthlyFee=@d4 where Year=@d2 and Months=@d1 and MultipleDate=@d3";
                cmd = new SqlCommand(cb);
                cmd.Connection = con;
                ...
                cmd.Parameters.Add(new SqlParameter("@d4", System.Data.SqlDbType.Int, 20, "MonthlyFee"));
