...
{
    public partial class frmShareToSavingsRecord : DevComponents.DotNetBar.Office2007Form
    {
        ...
        public frmShareToSavingsRecord()
        {
            InitializeComponent();
        }
        ...
        private void cmbStaffName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(MemberID)[Member ID],RTRIM(AccountNumber)[Account Number], RTRIM(Year)[Year], RTRIM(Months)[Months], RTRIM(Date)[Payment Date],RTRIM(CashierName)[Recieved By], RTRIM(TransferedAmmount)[Ammount Transfered] from SharesToSavings where MemberID= '" + cmbStaffName.Text + "' order by ID Desc", con);
                SqlDataAdapter myDA = new SqlDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "SharesToSavings");
                DGV.DataSource = myDataSet.Tables["SharesToSavings"].DefaultView;
                con.Close();
            }
            catch (Exception ex)
            {
        ...
        private void buttonX3_Click(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(MemberID)[Member ID],RTRIM(AccountNumber)[Account Number], RTRIM(Year)[Year], RTRIM(Months)[Months], RTRIM(Date)[Payment Date],RTRIM(CashierName)[Recieved By], RTRIM(TransferedAmmount)[Ammount Transfered] from SharesToSavings where  Date between @date1 and @date2 order by Date", con);
                ...
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "SharesToSavings");
                dataGridView1.DataSource = myDataSet.Tables["SharesToSavings"].DefaultView;
                con.Close();
            }
            catch (Exception ex)
            {
