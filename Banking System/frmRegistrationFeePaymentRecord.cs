...
{
    public partial class frmRegistrationFeePaymentRecord : DevComponents.DotNetBar.Office2007Form
    {
        ...
        public frmRegistrationFeePaymentRecord()
        {
            InitializeComponent();
        }
        ...
        private void cmbStaffName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(MemberID)[Member ID], RTRIM(Year)[Year], RTRIM(Months)[Months], RTRIM(Date)[Payment Date],RTRIM(CashierName)[Recieved By], RTRIM(RegistrationAmmount)[Ammount Paid], RTRIM(Duepayment)[Due Payment] from RegistrationFees where MemberID= '" + cmbStaffName.Text + "' order by ID Desc", con);
                SqlDataAdapter myDA = new SqlDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "RegistrationFees");
                DGV.DataSource = myDataSet.Tables["RegistrationFees"].DefaultView;
                con.Close();
            }
            catch (Exception ex)
            {
        ...
        private void buttonX3_Click(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(MemberID)[Member ID], RTRIM(Year)[Year], RTRIM(Months)[Months], RTRIM(Date)[Payment Date],RTRIM(CashierName)[Recieved By], RTRIM(RegistrationAmmount)[Ammount Paid], RTRIM(Duepayment)[Due Payment] from RegistrationFees where  Date between @date1 and @date2 order by Date", con);
