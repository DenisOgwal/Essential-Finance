...
{
    public partial class frmShareCapitalRecord : DevComponents.DotNetBar.Office2007Form
    {
        ...
        private void cmbStaffName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(MemberID)[Member ID], RTRIM(Year)[Year], RTRIM(Months)[Months], RTRIM(Date)[Payment Date],RTRIM(CashierName)[Recieved By], RTRIM(DepositedAmmount)[Ammount Paid], RTRIM(CurrentTotalShares)[Current Total Shares] from ShareCapital where MemberID= '" + cmbStaffName.Text + "' order by ID Desc", con);
                SqlDataAdapter myDA = new SqlDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "ShareCapital");
                DGV.DataSource = myDataSet.Tables["ShareCapital"].DefaultView;
                con.Close();
            }
            catch (Exception ex)
            {
        ...
        private void buttonX3_Click(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(MemberID)[Member ID], RTRIM(Year)[Year], RTRIM(Months)[Months], RTRIM(Date)[Payment Date],RTRIM(CashierName)[Recieved By], RTRIM(DepositedAmmount)[Ammount Paid], RTRIM(CurrentTotalShares)[Current Total Shares] from ShareCapital where  Date between @date1 and @date2 order by Date", con);
                ...
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "ShareCapital");
                dataGridView1.DataSource = myDataSet.Tables["ShareCapital"].DefaultView;
                con.Close();
            }
            catch (Exception ex)
            {
