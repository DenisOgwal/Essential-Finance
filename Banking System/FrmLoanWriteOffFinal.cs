...
{
    public partial class FrmLoanWriteOffFinal : DevComponents.DotNetBar.Office2007RibbonForm
    {
        ...
        private void FrmLoanFirstApproval_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select RTRIM(AccountNumber)[Account No.],RTRIM(AccountNames)[Account Name],RTRIM(LoanID)[Loan ID] from WriteOff where  Approval='Approved' and WriteoffDate ='Pending' order by ID DESC", con);
