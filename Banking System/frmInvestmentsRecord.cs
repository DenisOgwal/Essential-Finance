...
{
    public partial class frmInvestmentsRecord : DevComponents.DotNetBar.Office2007Form
    {
        ...
        private void cmbStaffName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(ReturnedBy)[Returned By], RTRIM(Year)[Year], RTRIM(Months)[Months], RTRIM(Date)[Payment Date],RTRIM(CashierName)[Recieved By], RTRIM(ReturnFee)[Returns],RTRIM(Investment)[Investment], RTRIM(Reason)[Reason] from InvestmentReturns where ReturnedBy= '" + cmbStaffName.Text + "' order by ID Desc", con);
                SqlDataAdapter myDA = new SqlDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "InvestmentReturns");
                DGV.DataSource = myDataSet.Tables["InvestmentReturns"].DefaultView;
                con.Close();
            }
            catch (Exception ex)
            {
        ...
        private void AutocompleteStaffName()
        {
            try
            {
                ...
                adp.SelectCommand = new SqlCommand("SELECT distinct RTRIM(ReturnedBy) FROM InvestmentReturns", CN);
                ds = new DataSet("ds");
                adp.Fill(ds);
                dtable = ds.Tables[0];
        ...
        private void buttonX3_Click(object sender, EventArgs e)
        {
            try
            {
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(ReturnedBy)[Returned By], RTRIM(Year)[Year], RTRIM(Months)[Months], RTRIM(Date)[Payment Date],RTRIM(CashierName)[Recieved By], RTRIM(ReturnFee)[Return Fee],RTRIM(Investment)[Investment], RTRIM(Reason)[Reason]from InvestmentReturns where  Date between @date1 and @date2 order by Date", con);
                ...
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "InvestmentReturns");
                dataGridView1.DataSource = myDataSet.Tables["InvestmentReturns"].DefaultView;
                con.Close();
            }
            catch (Exception ex)
            {
