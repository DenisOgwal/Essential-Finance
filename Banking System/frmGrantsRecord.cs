...
{
    public partial class frmGrantsRecord : DevComponents.DotNetBar.Office2007Form
    {
        ...
        private void cmbStaffName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select RTrim(PaymentID)[Payment ID], RTRIM(GrantedBy)[Granted By], RTRIM(Year)[Year], RTRIM(Months)[Months], RTRIM(Date)[Payment Date],RTRIM(CashierName)[Recieved By], RTRIM(GrantFee)[Grant Issued], RTRIM(Reason)[Reason]from GrantFees where GrantedBy= '" + cmbStaffName.Text + "' order by ID Desc", con);
                SqlDataAdapter myDA = new SqlDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "GrantFees");
                DGV.DataSource = myDataSet.Tables["GrantFees"].DefaultView;
                con.Close();
            }
            catch (Exception ex)
            {
        ...
        private void AutocompleteStaffName()
        {
            try
            {
                ...
                adp.SelectCommand = new SqlCommand("SELECT distinct RTRIM(GrantedBy) FROM GrantFees", CN);
                ds = new DataSet("ds");
                adp.Fill(ds);
                dtable = ds.Tables[0];
        ...
        private void buttonX3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "Grantfees");
                dataGridView1.DataSource = myDataSet.Tables["GrantFees"].DefaultView;
                con.Close();
            }
            catch (Exception ex)
            {
